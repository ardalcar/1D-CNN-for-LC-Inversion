...
# Carico modello
net=LSTMModel(input_size, hidden_size, output_size, num_layers)
net.to(device)
...
with torch.no_grad():
    loss = 0
    for input, lenghts, labels in val_loader:
        input = input.to(device)
        labels = labels.to(device)
        outputs = net(input, lenghts)
...
def test_accuracy(net, test_dataloader):
    ...
    reals=[]
    with torch.no_grad():
        for inputs, lengths, real in test_dataloader:
